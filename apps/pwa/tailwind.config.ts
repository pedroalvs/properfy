...
const config: Config = {
  ...
  theme: {
    extend: {
       ...
       * (`bg-white/92`) that scale is the only thing between the class and a working
       * rule, so the token helper above cannot save them. These are the steps this app
       * actually uses; `src/__tests__/opacity-scale.test.ts` fails if a new one appears.
       */
      opacity: {
        4: '0.04',
        6: '0.06',
