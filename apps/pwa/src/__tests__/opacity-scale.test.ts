 ...
 * *before* it looks at the colour at all. Tailwind's default scale is 0,5,…,100, so the
 * six `bg-white/92` profile cards (which rendered as bare outlines, their valid
 * `border-white/70` painting over nothing) were as dead as the token classes in #1041,
 * and for an entirely different reason: `white` is a real hex, so the `token()` helper in
 * `tailwind.config.ts` cannot save them.
 *
...
const COLOR_UTILITIES = [
  ...
  // is live in the web shell's SidebarSubmenu and a bare `border` alternative
  // does not match it.
  'border(?:-[tblrxyse])?',
  'ring(?:-offset)?',
  'from',
