...
export function createMockContainer(
  ...
  const base: AppContainer = {
    ...
    notification: {
      ...
      requestImageUploadUseCase: { execute: vi.fn() },
      confirmImageUploadUseCase: { execute: vi.fn() },
      listEmailAssetsUseCase: { execute: vi.fn() },
      editImageBindingUseCase: { execute: vi.fn() },
      deleteEmailAssetUseCase: { execute: vi.fn() },
    ...
    appCredential: {
      createAppCredentialUseCase: { execute: vi.fn() },
      updateAppCredentialUseCase: { execute: vi.fn() },
      getAppCredentialUseCase: { execute: vi.fn() },
      listAppCredentialsUseCase: { execute: vi.fn() },
      jwtService: { ...defaultJwt },
      tenantRepo: { ...defaultTenantRepo },
    } as AppContainer['appCredential'],
    integration: {
      listIntegrationsUseCase: { execute: vi.fn() },
      upsertIntegrationSettingUseCase: { execute: vi.fn() },
      deleteIntegrationSettingUseCase: { execute: vi.fn() },
      testIntegrationConnectionUseCase: { execute: vi.fn() },
      integrationConfigResolver: { getStatus: vi.fn(), getConfig: vi.fn(), invalidate: vi.fn() },
      createApiKeyUseCase: { execute: vi.fn() },
      listApiKeysUseCase: { execute: vi.fn() },
      revokeApiKeyUseCase: { execute: vi.fn() },
      jwtService: { ...defaultJwt },
      tenantRepo: { ...defaultTenantRepo },
    } as unknown as AppContainer['integration'],
    fy: {
      apiKeyAuthenticate: vi.fn(async () => {}),
      findFyAppointmentsByPhoneUseCase: { execute: vi.fn() },
      getFyAppointmentUseCase: { execute: vi.fn() },
      getFyAgencyUseCase: { execute: vi.fn() },
      getFyAvailableDatesUseCase: { execute: vi.fn() },
      addFyAppointmentNoteUseCase: { execute: vi.fn() },
      updateFyAppointmentContactUseCase: { execute: vi.fn() },
      resendFyNoticeUseCase: { execute: vi.fn() },
    } as unknown as AppContainer['fy'],
