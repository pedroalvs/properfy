...
describe('DownloadReportUseCase', () => {
  ...
  it.each(['INSP', 'TNT'] as const)('%s download is forbidden', async (role) => {
  ...
  it.each(['CL_ADMIN', 'CL_USER'] as const)('%s can download its own agency-scoped report', async (role) => {
  ...
  it('CL_ADMIN cannot download another agency report', async () => {
  ...
  // Same tenant_id, but generated by an operator — the file may carry the platform's
  // inspector-payout columns, so the agency must not be handed a presigned URL for it.
  it('CL_ADMIN cannot download an operator run targeting its own agency', async () => {
