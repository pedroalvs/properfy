...
import { normalizeImportRow } from '../../../src/modules/appointment/domain/appointment-import-normalize';
...
describe('real sample agency export', () => {
  it('parses every data row (48-row sheet = 1 header + 47 data rows)', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rows = await parseAppointmentImportFile(buffer, 'xlsx');
    expect(rows.length).toBe(47);
  });

  it('converts every row\'s Excel-serial Date to a valid YYYY-MM-DD without throwing', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
  ...
  it('restores a leading 0 on numeric-stored phone numbers', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
  ...
  it('normalizes every numeric postcode to a 4-digit string', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
    for (const raw of rawRows) {
      const { normalized } = normalizeImportRow(raw, IMPORT_DAY);
  ...
  it('keeps a multi-person Tenant name whole on the primary contact', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
  ...
  it('captures the sheet\'s own CUSTOM: Complete Property Address column as a custom field on every row', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
    for (const raw of rawRows) {
  ...
  it('every row has a usable street/suburb/state/postcode (no address-required errors expected downstream)', async () => {
    const buffer = readFileSync(FIXTURE_PATH);
    const rawRows = await parseAppointmentImportFile(buffer, 'xlsx');
    for (const raw of rawRows) {
      const { normalized } = normalizeImportRow(raw, IMPORT_DAY);
  ...
  it('most rows have a complete primary contact — one known gap (row 13, no phone)', async () => {
