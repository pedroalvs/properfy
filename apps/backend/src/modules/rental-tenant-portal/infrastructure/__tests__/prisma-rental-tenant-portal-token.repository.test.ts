...
import { PrismaRentalTenantPortalTokenRepository } from '../prisma-rental-tenant-portal-token.repository';

/**
 * Unit tests for PrismaRentalTenantPortalTokenRepository.findActiveByAppointmentId (T021)
 *
...
describe('PrismaRentalTenantPortalTokenRepository.findActiveByAppointmentId', () => {
  it('should return null when no ACTIVE token exists for the appointment', async () => {
    const prisma = {
      rentalTenantPortalToken: { findFirst: vi.fn().mockResolvedValue(null) },
    } as any;
    const repo = new PrismaRentalTenantPortalTokenRepository(prisma);
  ...
  it('should return entity when ACTIVE token has expires_at > now()', async () => {
    ...
    const prisma = {
      rentalTenantPortalToken: { findFirst: vi.fn().mockResolvedValue(tokenRow) },
    } as any;
    const repo = new PrismaRentalTenantPortalTokenRepository(prisma);
  ...
  it('should include expires_at > now() predicate in the WHERE clause', async () => {
    ...
    expect(prisma.rentalTenantPortalToken.findFirst).toHaveBeenCalledOnce();
    const callArgs = prisma.rentalTenantPortalToken.findFirst.mock.calls[0][0];
