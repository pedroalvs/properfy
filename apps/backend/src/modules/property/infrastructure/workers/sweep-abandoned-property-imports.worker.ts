...
/**
 * Scheduled cleanup: an operator who previews a property import but never
 * commits leaves a `PREVIEW` row plus its uploaded file behind forever.
 * Deletes both once older than `maxAgeHours`. Mirrors
 * `SweepAbandonedAppointmentImportsWorker`.
 */
export class SweepAbandonedPropertyImportsWorker {
  constructor(
  ...
  async execute(): Promise<{ sweptCount: number }> {
    ...
    if (sweptCount > 0) {
      this.logger.info({ sweptCount }, 'Swept abandoned property-import previews');
    }
    return { sweptCount };
  }
}
