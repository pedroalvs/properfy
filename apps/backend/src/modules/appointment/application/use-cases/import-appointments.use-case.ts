...
export class ImportAppointmentsUseCase {
  ...
  async execute(input: ImportAppointmentsInput): Promise<ImportAppointmentsOutput> {
    ...
    // Idempotency check
    const cached = await this.idempotencyService.get<ImportAppointmentsOutput>(idempotencyKey, 'appointment.import');
    if (cached) {
      return cached;
    }
    ...
    await this.idempotencyService.set(idempotencyKey, 'appointment.import', result, 24);
