...
1. **Board/Kanban:** now IN SCOPE and **shipped** — explicitly requested by the client (see `escopo-pendencias-execucao.md` item 09, "Service Dashboard" in the client scope doc §4.3). Column-per-status board for AM/OP at `/appointments/board`, reachable from the "Board" action on the appointments list.
   ...
   - **`overdueOnly` composes, it does not override.** `buildWhere` intersects it with both the caller's `status` and the caller's `fromDate`/`toDate` (the tighter upper bound wins). It used to *replace* both, which rendered the same appointment in two columns and made the Period filter a silent no-op. The board still skips the three terminal column queries while the filter is on, now purely as an optimisation — the intersection would return them empty anyway. If you touch this, keep the intersection: a replace re-breaks column-per-status.
