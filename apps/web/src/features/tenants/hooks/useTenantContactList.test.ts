...
const MOCK_CONTACTS = [
  { id: 'tnt-01', name: 'Ana Silva', email: 'ana.silva@email.com', confirmationStatus: 'CONFIRMED' },
  { id: 'tnt-02', name: 'Bruno Santos', email: 'bruno@email.com', confirmationStatus: 'NO_RESPONSE' },
];
...
describe('useTenantContactList', () => {
  it('returns data after loading resolves', async () => {
    ...
    const { result } = renderHook(() => useTenantContactList(), { wrapper });
  ...
  it('calls API with correct path', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith('/v1/tenants', expect.any(Object));
  });
