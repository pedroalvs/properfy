...
async function mockAvailableGroups(
  ...
  await page.route(`**/v1/rental-tenant-portal/${TOKEN}/available-groups`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
...
async function mockJoinGroup(page: Page, status = 200) {
  await page.route(`**/v1/rental-tenant-portal/${TOKEN}/join-group`, async (route) => {
    if (status === 200) {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  test('happy path: select group and join', async ({ page }) => {
    ...
    await page.route(`**/v1/rental-tenant-portal/${TOKEN}`, async (route) => {
      ...
      const body = callCount === 1
        ? { ...portalData }
        : { ...portalData, appointment: { ...portalData.appointment, rentalTenantConfirmationStatus: 'CONFIRMED' } };
