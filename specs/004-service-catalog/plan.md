...
| II. Multi-Tenant Safety | PARTIAL — CORRECTION-004 pending | Service type is an intentional exception: global catalog curated by AM only — documented in the spec. Pricing rule scopes by tenant on every read/write; CL_ADMIN cannot set foreign `tenantId`. **Service region is tenant-scoped per the approved dossiê** (CORRECTION-004); the code currently stores regions without `tenant_id`, which is a divergence that must be corrected. Until corrected, region access is guarded by AM/OP at the use-case layer but lacks tenant isolation. |
