...
- [x] T020 [US4] ~~Create `apps/backend/src/modules/contact/application/use-cases/search-contacts.use-case.ts`~~ **Superseded by DEC-011** — search is integrated into `ListContactsUseCase` via the `search` query parameter. `IContactRepository.search()` is called when `search` is present in `findAll()` filters; no separate use case exists or is needed.
