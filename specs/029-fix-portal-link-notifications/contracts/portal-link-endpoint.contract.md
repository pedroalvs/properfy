...
- **404 NoActivePortalTokenError** if no such token row exists.
...
| TC-PL-3 | `active_confirmation_cycle_id IS NOT NULL` AND no token row | 404 NoActivePortalTokenError |
| TC-PL-4 | `active_confirmation_cycle_id IS NOT NULL` AND token REVOKED | 404 NoActivePortalTokenError |
| TC-PL-5 | `active_confirmation_cycle_id IS NOT NULL` AND token ACTIVE but `expires_at <= NOW` | 404 NoActivePortalTokenError (covered jointly by T028 + this change) |
