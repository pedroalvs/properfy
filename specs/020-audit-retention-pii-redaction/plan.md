...
| II. Multi-Tenant Safety | PASS | The audit table is intentionally cross-tenant (AM sees everything). OP is scoped to own tenant. CL_ADMIN is scoped to own tenant. Erasure requests and legal holds are tenant-aware where applicable — the preservation rule for a legal hold carries an optional `tenant_id` to prevent cross-tenant leakage on platform-scoped holds. |
